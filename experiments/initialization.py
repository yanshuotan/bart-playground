...
def run_chain_bart(X_train_raw, X_test_raw, y_train_raw, y_test_raw,
    ...
    model = DefaultBART(
        ...
        random_state=seed,
        proposal_probs=default_proposal_probs,  # Consider making this configurable
...
def run_full_experiment(X_train_specific, y_train_specific, X_test_specific, y_test_specific,
                        cfg: DictConfig, init_from_xgb: bool):
...
# --- Plotting Functions ---
def plot_results(results_scratch, results_xgb, cfg: DictConfig, plot_dir_for_run: str, n_train_samples: int):
    os.makedirs(plot_dir_for_run, exist_ok=True)

    # Bar plots for overall metrics
    metrics_to_plot = ['final_mse', 'coverage', 'runtime']
    labels = ['FromScratch', 'XGBoostInit']

    for metric in metrics_to_plot:
        values = [results_scratch[metric], results_xgb[metric]]
    ...
    # Diagnostic plots
    diag_cfg_map = {
        "FromScratch": results_scratch,
        "XGBoostInit": results_xgb
    }
    # max_lag_acf = cfg.experiment_params.get("max_lag_acf", 50) # No longer needed
...
def run_and_analyze(cfg: DictConfig):
    ...
    dgp_params_test_dict['noise'] = 0.0  # Critical: noise = 0.0 for true y_test
    ...
    LOGGER.info(
        f"Generating test data (noise=0): N_test={exp_params.n_test_samples}, DGP params: {dgp_params_test_dict}")
    ...
    for idx, current_n_train in enumerate(exp_params.n_train_samples_list):
        ...
        # Noise for training data comes from the config
        # dgp_params_train_dict['noise'] is already cfg.dgp_params.noise
        ...
        # Save config for this specific run (optional, but good for traceability)
        # Could also save a subset of the config relevant to this run.
        # OmegaConf.save(cfg, os.path.join(run_specific_artifact_dir, "config_run.yaml"))
        ...
        LOGGER.info(f"Results (FromScratch) for DGP {cfg.dgp}, N_train={current_n_train}:")
        ...
        LOGGER.info(f"Results (XGBoostInit) for DGP {cfg.dgp}, N_train={current_n_train}:")
        ...
        if exp_params.get("plot_results", True):
            ...
            plot_results(results_scratch, results_xgb, cfg, run_specific_artifact_dir, current_n_train)

        LOGGER.info(
            f"Experiment for DGP {cfg.dgp} with N_train={current_n_train} complete. Artifacts saved to {run_specific_artifact_dir}")


@hydra.main(config_path="configs", config_name="initialization", version_base=None)
