...
def test_linear_ts_posterior_equivalence(n_arms, n_features, steps, v, seed):
    ...
    aggr = LinearAgentStable(agent_type=AgentType.TS, n_arms=n_arms, n_features=n_features, v=v, random_state=seed)
    ...
    for a in range(n_arms):
        ...
        theta_aggr = aggr._get_post_mean(a).reshape(-1)
        ...
        B_inv_aggr = np.linalg.inv(aggr.B[a])
        ...
        C1 = np.linalg.solve(aggr.L[a].T, np.eye(d))
...
def test_linear_ts_score_distribution_stats(n_arms, n_features, steps, v, seed):
    ...
    for a in range(n_arms):
        theta_aggr = aggr._get_post_mean(a).reshape(-1, 1)
        theta_stab = np.linalg.solve(stab.B[a], stab.m2_r[a]).reshape(-1, 1)
        B_inv = np.linalg.inv(aggr.B[a])
