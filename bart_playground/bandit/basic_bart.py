...
class BARTTSAgent(BanditAgent):
    ...
    """
    def __init__(self, n_arms: int, n_features: int, nskip: int = 50) -> None:
        """
        ...
        self.features: List[np.ndarray] = [np.empty((0, n_features)) for _ in range(n_arms)]
        self.outcomes: List[np.ndarray] = [np.empty((0, 1)) for _ in range(n_arms)]
        self.nskip_default: int = nskip

    def choose_arm(self, x: Union[np.ndarray, List[float]], nskip: Optional[int] = None, binary: bool = False) -> int:
        """
        ...
        u: np.ndarray = np.zeros(self.n_arms)
        for arm in range(self.n_arms):
            fts = self.features[arm]
    ...
    def update_state(self, arm: int, x: Union[np.ndarray, List[float]], y: Union[float, np.ndarray]) -> "BARTTSAgent":
        """
