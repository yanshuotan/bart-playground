...
def generate_data_from_defaultbart_prior(
    ...
    *,
    ...
    """
    Generate regression data according to the DefaultBART prior, with X ∈ [0,1]^p.
    ...
    # 1) X ~ Uniform([0,1]^p)
    X = rng.uniform(0.0, 1.0, size=(n, p)).astype(np.float32)
        ...
        while queue:
            ...
            for _ in range(32):
                ...
                if is_valid and (t.n[node_id * 2 + 1] >= min_node_size) and (t.n[node_id * 2 + 2] >= min_node_size):
                    success = True
                    break
                # revert failed split (prune back if created) – if invalid, the helper didn't finalize counts,
...
def generate_data_heatmap(**kwargs) -> str:
    ...
    """
    res = generate_data_from_defaultbart_prior(p=2, return_latent=True, **kwargs)
    ...
    out_path = os.path.join(out_dir, 'generator_heatmap.png')
