...
class BCFPrior:
    ...
    def __init__(self, n_treat_arms : int, n_mu_trees=200, n_tau_trees : list[int] = [],
        ...
        self.tau_prior_list = [TreesPrior(
            n_trees=n_tau_trees[i],
            tree_alpha=tau_alpha,
            tree_beta=tau_beta,
            f_k=tau_k,
            generator=generator
        ) for i in range(n_treat_arms)]
        ...
        self.tau_likelihood_list = [BARTLikelihood(self.tau_prior_list[i].f_sigma2) for i in range(n_treat_arms)]
      
    def trees_log_prior_ratio(self, move : Move, ensemble_id : BCFEnsembleIndex):
        if ensemble_id.ensemble_name == EnsembleName.MU:
            bart_prior : TreesPrior = self.mu_prior
        else:
            bart_prior : TreesPrior = self.tau_prior_list[ensemble_id.index]
    ...
    def trees_log_marginal_lkhd_ratio(self, move : Move, data_y, ensemble_id : BCFEnsembleIndex, marginalize: bool=False):
        """
        ...
        - data_y: np.ndarray
            The data to compute the marginal likelihood ratio for.
        - ensemble_id: BCFEnsembleIndex
            The ensemble to compute the marginal likelihood ratio for.
        - marginalize: bool
        ...
        if ensemble_id.ensemble_name == EnsembleName.MU:
            ...
            bart_likelihood : BARTLikelihood = self.tau_likelihood_list[ensemble_id.index]
