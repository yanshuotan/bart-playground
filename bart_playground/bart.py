...
class BART:
    ...
    def __init__(
        self, preprocessor: Preprocessor, sampler: Sampler, ndpost=1000, nskip=100
    ):
        """
    ...
    def fit(self, X, y, quietly=False):
        ...
        self.trace = self.sampler.run(
            self.ndpost + self.nskip, quietly=quietly, n_skip=self.nskip
        )
        self.is_fitted = True
    ...
    def update_fit(self, X, y, add_ndpost=20, add_nskip=10, quietly=False):
        ...
        new_trace = self.sampler.continue_run(
            additional_iters, new_data=self.data, quietly=quietly
        )
    ...
    def posterior_predict(self, X):
        ...
        for k in range(self.ndpost):
            ...
            preds[:, k] += self.sampler.generator.normal(
                0, np.sqrt(eps_sigma2), size=preds[:, k].shape
            )
    ...
    def init_from_xgboost(
        ...
        if self.data is None:
            ...
            X is not self.data.X or y is not self.data.y
        ):  # Check if X,y are different objects
            ...
            print(
                "[WARN BART.init_from_xgboost] X or y are different objects than self.data.X/y. Calling update_transform."
            )
            self.data = self.preprocessor.update_transform(X, y, self.data)
        ...
        if debug:  # Check if debug flag is True
            ...
            print(
                f"[DEBUG XGB_INIT]   Tree 0 Leaf Vals (from XGB): {init_state.trees[0].leaf_vals[init_state.trees[0].leaves]}"
            )
            print(
                f"[DEBUG XGB_INIT]   Global eps_sigma2: {init_state.global_params['eps_sigma2']}"
            )
        ...
        for k in range(self.sampler.tree_prior.n_trees):
            ...
            if debug:
                print(
                    f"[DEBUG XGB_INIT] Resampled Leaf Vals for tree {k}: {new_leaf_vals}"
                )
        ...
        init_state.global_params = self.sampler.global_prior.resample_global_params(
            init_state, data_y=self.data.y
        )
        if debug:
            print(
                f"[DEBUG XGB_INIT] Resampled Global eps_sigma2: {init_state.global_params['eps_sigma2']}"
            )
            print(
                f"[DEBUG XGB_INIT] Final state for trace - Tree 0 Leaf Vals: {init_state.trees[0].leaf_vals[init_state.trees[0].leaves]}"
            )
...
class ProbitBART(BART):
    ...
    def posterior_predict(self, X):
        ...
        draws = self.sampler.generator.binomial(
            1, prob_samples, size=prob_samples.shape
        ).astype(int)
...
class LogisticBART(BART):
    ...
    def posterior_predict(self, X):
        ...
        draws = self.sampler.generator.multinomial(
            n=1, pvals=prob_samples, size=(prob_samples.shape[0], prob_samples.shape[1])
        )
