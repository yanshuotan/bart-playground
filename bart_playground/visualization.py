...
def visualize_tree(tree, filename: str = "tree", format: str = "png"):
    """
    Visualize a tree structure using Graphviz.

    Parameters:
    - tree: Tree
        The Tree class from params.py
    - filename: str
    ...
    def add_nodes_edges(node_id):
        ...
        if tree.vars[node_id] == -1:
            ...
            if left_child < len(tree.vars) and tree.vars[left_child] != -2:
                dot.edge(str(node_id), str(left_child), label="Left")
            if right_child < len(tree.vars) and tree.vars[right_child] != -2:
