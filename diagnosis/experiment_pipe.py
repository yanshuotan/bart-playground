from joblib import Parallel, delayed
from sklearn.model_selection import train_test_split
from sklearn.metrics import root_mean_squared_error
import numpy as np
from bart_playground import *

def count_leaves_in_trees(trace_record):
    """Count leaves (vars == -1) in all trees of a single trace record and return average"""
    total_leaves = 0
    total_trees = len(trace_record.trees)
    
    for tree in trace_record.trees:
        # Count number of -1s in vars (leaf nodes)
        leaves_count = np.sum(np.array(tree.vars) == -1)
        total_leaves += leaves_count
    
    # Return average number of leaves per tree
    return total_leaves / total_trees

def calculate_tree_depth(tree):
    """Calculate tree depth as ceil(log2(position_of_last_-1)) - 1"""
    vars_array = np.array(tree.vars)
    
    # Find positions of all -1s (leaf nodes)
    leaf_positions = np.where(vars_array == -1)[0]
    
    # Get the last position of -1
    last_leaf_position = leaf_positions[-1]
    
    # Calculate depth
    depth = int(np.ceil(np.log2(last_leaf_position + 2))) - 1
    
    return depth

def calculate_depth_per_trace(trace_record):
    """Calculate average tree depth for all trees in a trace record"""
    total_depth = 0
    total_trees = len(trace_record.trees)

    tree_depths = [calculate_tree_depth(tree) for tree in trace_record.trees]
    total_depth = np.sum(tree_depths)
    max_depth = np.max(tree_depths)

    return total_depth / total_trees, max_depth

def run_experiment(run_id, X, y, ndpost, nskip, n_trees, m_tries, tree_alpha, tree_beta, multi_iter=4000):
    """Run a single experiment with different train-test split"""
    
    # Use different random_state for train test split
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=run_id)

    proposal_probs_default = {
        'grow': 0.25,
        'prune': 0.25,
        'change': 0.4,
        'swap': 0.1
    }

    proposal_probs_mtmh = {
        'multi_grow': 0.25,
        'multi_prune': 0.25,
        'multi_change': 0.4,
        'multi_swap': 0.1
    }

    # Train pipeline BART model (mtmh -> default)
    pipeline_bart = PipelineBART(ndpost=ndpost-multi_iter, nskip=nskip, n_trees=n_trees, 
                                 multi_proposal_probs=proposal_probs_mtmh,
                                 proposal_probs=proposal_probs_default,
                                 tree_alpha=tree_alpha, tree_beta=tree_beta,
                                 multi_tries=m_tries, random_state=0)
    pipeline_bart.fit(X_train, y_train, multi_iter=multi_iter)

    # Extract pipeline BART results
    sigmas_pipeline = [trace.global_params['eps_sigma2'] for trace in pipeline_bart.trace]
    preds_pipeline = pipeline_bart.posterior_f(X_test, backtransform=True)
    rmses_pipeline = [root_mean_squared_error(y_test, preds_pipeline[:, k]) for k in range(preds_pipeline.shape[1])]
    leaves_pipeline = [count_leaves_in_trees(trace) for trace in pipeline_bart.trace]
    depths_pipeline = [calculate_depth_per_trace(trace) for trace in pipeline_bart.trace]
    avg_depths_pipeline = [depth[0] for depth in depths_pipeline]
    max_depths_pipeline = [depth[1] for depth in depths_pipeline]

    # Train default BART model
    bart_default = DefaultBART(ndpost=ndpost, nskip=nskip, n_trees=n_trees,
                    proposal_probs=proposal_probs_default, random_state=0)
    bart_default.fit(X_train, y_train)
    
    # Extract default BART results
    sigmas_default = [trace.global_params['eps_sigma2'] for trace in bart_default.trace]
    preds_default = bart_default.posterior_f(X_test, backtransform=True)
    rmses_default = [root_mean_squared_error(y_test, preds_default[:, k]) for k in range(preds_default.shape[1])]
    leaves_default = [count_leaves_in_trees(trace) for trace in bart_default.trace]
    depths_default = [calculate_depth_per_trace(trace) for trace in bart_default.trace]
    avg_depths_default = [depth[0] for depth in depths_default]
    max_depths_default = [depth[1] for depth in depths_default]

    # Train MTMH BART model
    bart_mtmh = MultiBART(ndpost=ndpost, nskip=nskip, n_trees=n_trees,
                    proposal_probs=proposal_probs_mtmh, multi_tries=m_tries, 
                    tree_alpha=tree_alpha, tree_beta=tree_beta, # Only for mtmh prior
                    random_state=0)
    bart_mtmh.fit(X_train, y_train)
    
    # Extract MTMH BART results
    sigmas_mtmh = [trace.global_params['eps_sigma2'] for trace in bart_mtmh.trace]
    preds_mtmh = bart_mtmh.posterior_f(X_test, backtransform=True)
    rmses_mtmh = [root_mean_squared_error(y_test, preds_mtmh[:, k]) for k in range(preds_mtmh.shape[1])]
    leaves_mtmh = [count_leaves_in_trees(trace) for trace in bart_mtmh.trace]
    depths_mtmh = [calculate_depth_per_trace(trace) for trace in bart_mtmh.trace]
    avg_depths_mtmh = [depth[0] for depth in depths_mtmh]
    max_depths_mtmh = [depth[1] for depth in depths_mtmh]
    
    # Return results as dictionary instead of saving individual files
    return {
        'run_id': run_id,
        'default': {
            'sigmas': np.array(sigmas_default),
            'preds': np.array(preds_default),
            'rmses': np.array(rmses_default),
            'leaves': np.array(leaves_default),
            'avg_depths': np.array(avg_depths_default),
            'max_depths': np.array(max_depths_default)
        },
        'mtmh': {
            'sigmas': np.array(sigmas_mtmh),
            'preds': np.array(preds_mtmh),
            'rmses': np.array(rmses_mtmh),
            'leaves': np.array(leaves_mtmh),
            'avg_depths': np.array(avg_depths_mtmh),
            'max_depths': np.array(max_depths_mtmh)
        },
        'pipeline': {
            'sigmas': np.array(sigmas_pipeline),
            'preds': np.array(preds_pipeline),
            'rmses': np.array(rmses_pipeline),
            'leaves': np.array(leaves_pipeline),
            'avg_depths': np.array(avg_depths_pipeline),
            'max_depths': np.array(max_depths_pipeline)
        }
    }

def run_parallel_experiments(X, y, ndpost, nskip, multi_iter, n_trees, notebook, tree_alpha=0.95, tree_beta=2.0, m_tries=10, n_runs=5, n_jobs=-1):
    """Run parallel experiments with different train-test splits"""
    
    results = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(run_experiment)(run_id, X, y, ndpost, nskip, n_trees, m_tries, tree_alpha, tree_beta, multi_iter)
        for run_id in range(n_runs)
    )
    
    # Combine all results into structured arrays
    combined_results = {
        'default': {
            'sigmas': np.array([r['default']['sigmas'] for r in results]),
            'preds': np.array([r['default']['preds'] for r in results]),
            'rmses': np.array([r['default']['rmses'] for r in results]),
            'leaves': np.array([r['default']['leaves'] for r in results]),
            'avg_depths': np.array([r['default']['avg_depths'] for r in results]),
            'max_depths': np.array([r['default']['max_depths'] for r in results])
        },
        'mtmh': {
            'sigmas': np.array([r['mtmh']['sigmas'] for r in results]),
            'preds': np.array([r['mtmh']['preds'] for r in results]),
            'rmses': np.array([r['mtmh']['rmses'] for r in results]),
            'leaves': np.array([r['mtmh']['leaves'] for r in results]),
            'avg_depths': np.array([r['mtmh']['avg_depths'] for r in results]),
            'max_depths': np.array([r['mtmh']['max_depths'] for r in results])
        },
        'pipeline': {
            'sigmas': np.array([r['pipeline']['sigmas'] for r in results]),
            'preds': np.array([r['pipeline']['preds'] for r in results]),
            'rmses': np.array([r['pipeline']['rmses'] for r in results]),
            'leaves': np.array([r['pipeline']['leaves'] for r in results]),
            'avg_depths': np.array([r['pipeline']['avg_depths'] for r in results]),
            'max_depths': np.array([r['pipeline']['max_depths'] for r in results])
        },
        'metadata': {
            'n_runs': n_runs,
            'ndpost': ndpost,
            'nskip': nskip,
            'n_trees': n_trees,
            'm_tries': m_tries
        }
    }
    
    # Save as single compressed file
    np.savez_compressed(f'store/{notebook}.npz', **combined_results)
    
    return results