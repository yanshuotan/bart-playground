...
from bart_playground import DefaultBART, ParallelTemperingBART
...
def run_chain(
    *,
    ...
    """Run a default-BART chain and a PT-BART chain on the same train/test split."""
    ...
    default_preds = bart_default.posterior_f(X_test, backtransform=True)
    default_sigmas = [trace.global_params["eps_sigma2"] for trace in bart_default.sampler.trace]
    default_rmses = [
        root_mean_squared_error(y_test, default_preds[:, k]) for k in range(default_preds.shape[1])
    ]
    default_leaves = [count_leaves_in_trees(trace) for trace in bart_default.sampler.trace]
    default_depths = [calculate_avg_depth_per_trace(trace) for trace in bart_default.sampler.trace]
    default_accepted_moves = np.array(bart_default.sampler.accepted_moves_logmh, dtype=object)

    default_pred_all_test = bart_default.posterior_predict(X_test)
    ...
    default_covered_bool = (y_test >= default_lower) & (y_test <= default_upper)

    del bart_default
    gc.collect()

    bart_pt = ParallelTemperingBART(
        ndpost=ndpost,
        nskip=nskip,
        n_trees=n_trees,
        tree_alpha=tree_alpha,
        tree_beta=tree_beta,
        tol=1,
        proposal_probs=proposal_probs_pt,
        random_state=chain_seed,
    ...
    bart_pt.fit(X_train, y_train)

    pt_preds = bart_pt.posterior_f(X_test, backtransform=True)
    pt_sigmas = [trace.global_params["eps_sigma2"] for trace in bart_pt.trace]
    pt_rmses = [root_mean_squared_error(y_test, pt_preds[:, k]) for k in range(pt_preds.shape[1])]
    pt_leaves = [count_leaves_in_trees(trace) for trace in bart_pt.trace]
    pt_depths = [calculate_avg_depth_per_trace(trace) for trace in bart_pt.trace]
    pt_accepted_moves = np.array(bart_pt.sampler.accepted_moves_logmh, dtype=object)
    pt_swap_accept_rates = np.asarray(bart_pt.get_params().get("swap_accept_rates", []), dtype=float)

    pt_pred_all_test = bart_pt.posterior_predict(X_test)
    ...
    pt_pred_subsample = pt_pred_all_test[idx, :]

    default_subsample_rmse = root_mean_squared_error(y_test_subsample, np.mean(default_pred_subsample, axis=1))
    pt_subsample_rmse = root_mean_squared_error(y_test_subsample, np.mean(pt_pred_subsample, axis=1))
    default_subsample_crps = float(np.mean(crps_from_samples(default_pred_subsample, y_test_subsample)))
    pt_subsample_crps = float(np.mean(crps_from_samples(pt_pred_subsample, y_test_subsample)))
    ...
    result = {
        ...
        "default": {
            "sigmas": np.array(default_sigmas),
            "rmses": np.array(default_rmses),
            "leaves": np.array(default_leaves),
            "depths": np.array(default_depths),
            "accepted_moves_logmh": default_accepted_moves,
        ...
        "pt": {
            ...
            "accepted_moves_logmh": pt_accepted_moves,
            "subsample_rmse": pt_subsample_rmse,
            "subsample_crps": pt_subsample_crps,
            "swap_accept_rates": pt_swap_accept_rates,
        },
    }

    if store_preds:
        result["default"]["preds"] = np.array(default_preds[idx])
        result["default"]["coverage"] = np.array(default_covered_bool[idx])
        result["pt"]["preds"] = np.array(pt_preds[idx])
        result["pt"]["coverage"] = np.array(pt_covered_bool[idx])
...
def _stack_run_results(
    *,
    ...
    run_result = {
        ...
        "pt": {
            "sigmas": np.array([r["pt"]["sigmas"] for r in chain_results]),
            "rmses": np.array([r["pt"]["rmses"] for r in chain_results]),
            "leaves": np.array([r["pt"]["leaves"] for r in chain_results]),
            "depths": np.array([r["pt"]["depths"] for r in chain_results]),
            "accepted_moves_logmh": np.array(
                [r["pt"]["accepted_moves_logmh"] for r in chain_results],
                dtype=object,
            ),
            "subsample_rmse": np.array([r["pt"]["subsample_rmse"] for r in chain_results]),
            "subsample_crps": np.array([r["pt"]["subsample_crps"] for r in chain_results]),
            "swap_accept_rates": np.array([r["pt"]["swap_accept_rates"] for r in chain_results]),
        },
        "metadata": {
            "n_chains": n_chains,
            "ndpost": ndpost,
    ...
    if "preds" in chain_results[0]["default"]:
        ...
        run_result["pt"]["preds"] = np.array([r["pt"]["preds"] for r in chain_results])
        run_result["pt"]["coverage"] = np.array([r["pt"]["coverage"] for r in chain_results])
    ...
    for run_offset in range(n_runs):
        ...
        chain_results = Parallel(n_jobs=n_jobs, verbose=10)(
            delayed(run_chain)(
                ...
                proposal_probs_pt=proposal_probs_pt,
