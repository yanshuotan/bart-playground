...
def summarize_model_outputs(model, X_test_fixed, y_test_fixed, *, store_preds: bool):
    ...
    result = {
        ...
        "subsample_rmse": root_mean_squared_error(y_test_fixed, np.mean(preds, axis=1)),
        "subsample_crps": float(np.mean(crps_from_samples(preds, y_test_fixed))),
    }
...
def save_short_run(
    *,
    ...
    for sub in ["preds", "coverage", "sigmas", "rmses", "leaves", "depths", "accepted_moves_logmh", "subsample_rmse", "subsample_crps", "swap_accept_rates", "subsample_X_test", "subsample_y_test", "indices", "metadata"]:
