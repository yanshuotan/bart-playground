...
Changes relative to the public fixed100 runner:
  ...
  - max ladder size cap is disabled by default
  - memory logger is disabled by default on macOS to avoid BSD ps warnings
  - formal defaults match the sparse rerun: 10 rounds, 3 repeats,
    nskip=500, ndpost=500, and 1,000 search points
  - formal reruns use 2 runs, each with 4 chains x 10,000 iterations per chain
...
def main():
    ...
    parser.add_argument("--ladder-max-temperatures", type=int, default=0, help="Use 0 to disable the cap.")
    ...
    if not args.skip_long:
        parser.error(
            "This formal rerun wrapper is short-only; pass --skip-long explicitly."
        )
    if args.short_ndpost != 10_000 or args.n_chains != 4:
        parser.error(
    ...
    if args.preflight_only:
        print("PREFLIGHT ONLY: no ladder search or model fitting will run", flush=True)
        for position, name in enumerate(args.datasets, start=1):
            X, y = load_dataset(name)
            if X.ndim != 2 or y.ndim != 1 or X.shape[0] != y.shape[0]:
                raise ValueError(
                    f"Invalid loaded shape for {name}: X={X.shape}, y={y.shape}"
                )
            if not np.isfinite(X).all() or not np.isfinite(y).all():
                raise ValueError(f"Non-finite values remain after preprocessing: {name}")
            print(
                f"[{position}/{len(args.datasets)}] {name}: "
                f"X={X.shape}, y={y.shape}, dataset_tag={DATASET_CONFIGS[name]['dataset_tag']}",
                flush=True,
            )
        print(
            "PREFLIGHT PASSED; formal execution will process these datasets "
            "sequentially in the displayed order",
            flush=True,
        )
        return
