...
def main():
    ...
    parser.add_argument("--swap-sigma", choices=["true", "false", "both"], default="both")
    ...
    print(f"swap_sigma={args.swap_sigma}, skip_long={args.skip_long}", flush=True)
    ...
    try:
        for name in args.datasets:
            ...
            swap_modes = [True, False] if args.swap_sigma == "both" else [args.swap_sigma == "true"]
            for swap_sigma in swap_modes:
                suffix = "swap_sigma_true" if swap_sigma else "swap_sigma_false"
                print(f"[{name}] swap_sigma={swap_sigma}", flush=True)
                run_fixed100_dataset(
                    X=X,
                    y=y,
                    dataset_tag=f"{cfg['dataset_tag']}__{suffix}",
