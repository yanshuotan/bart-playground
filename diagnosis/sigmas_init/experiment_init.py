...
def run_experiment(run_id, X, y, ndpost, nskip, n_trees, m_tries, 
    ...
    # # Fit linear regression to get sigma
    # linreg = LinearRegression()
    # linreg.fit(X_train, y_train)
    # y_pred = linreg.predict(X_train)
    # sigma = np.std(y_train - y_pred)

    sigma = 0.01 # Hardcoded small sigma
    ...
    bart_mtmh = MultiBART(ndpost=ndpost, nskip=nskip, n_trees=n_trees,
                          ...
                          random_state=0, init_trees=random_trees, init_sigma2=[sigma])
...
def run_parallel_experiments(X, y, ndpost, nskip, n_trees, notebook, 
                             ...
                             n_runs=5, n_jobs=-1, store_preds=False, n_test_points=10):
