...
def run_experiment(run_id, chain_id, X, y, ndpost, nskip, n_trees, m_tries, temperature, 
    ...
    if run_id == 0:
        init_trees = None # Use default initialization (roots) for run_id 0
    else:
    ...
    bart_mtmh = MultiBART(ndpost=ndpost, nskip=nskip, n_trees=n_trees,
                          ...
                          random_state=run_id*100+chain_id, init_trees=init_trees)
...
def run_parallel_experiments(X, y, ndpost, nskip, n_trees, notebook, 
    ...
    if store_preds:
        ...
        combined_results['default']['coverage'] = np.array([[chain['default']['coverage'] for chain in run] for run in results])
        combined_results['mtmh']['coverage'] = np.array([[chain['mtmh']['coverage'] for chain in run] for run in results])
